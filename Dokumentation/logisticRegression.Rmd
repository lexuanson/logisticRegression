...
$$ 
Wobei $\mathbf{x}_i$ der i-ten Zeile der Designmatrix $\mathbf{X}$ entspricht.
...
&= - \sum_{i = 1}^{n} \Bigg[ \mathbf{x}_i \ . \frac{\mathbf{x}_i.\exp^2(\mathbf{x}_i\beta)}{(1+\exp(\mathbf{x}_i.\beta))^2}  \Bigg] \\
&\mathrel{\overset{(3)}{=}} - \sum_{i = 1}^{n} \Bigg[ \mathbf{x}_i \ . \frac{\mathbf{x}_i}{1+\exp(\mathbf{x}_i.\beta)} . .... \Bigg] \\
\end{aligned}
$$
