...
```{r eval = FALSE, results = 'hide', warning = FALSE, message = FALSE, tidy=TRUE}
...
Solange die beiden Abbruchskriterien nicht erreicht sind, erfolgt in der Newton-Raphson-Schleife ein iterativer Vorgang, welcher sich auf dem erklärten Pseudocode (siehe Abschnitt ) basiert. Nachdem der ML-Schätzer bestimmt wird, werden folgende Parameter berechnet. Diese Werte werden hierbei berechnet, um die Unabhängigkeit der S3-Methoden voneinander zu garantieren. Das bedeutet, dass beispielsweise *summary()* ohne *print()* aufgerufen werden kann.  
...
* **Deviance Residuals**: Die Devianzresiduen lassen sich anhand der folgenden Formel[^cite01] berechnet:
$$
...
* **Kovarianzmatrix**: die Kovarianzmatrix lässt wie folgt berechnen:
...
In der Summary-Methode werden zunächst lediglich bisher noch fehlende Werte berechnet:
* **Standard Error** der Koeffizienten
* **z-Stat**
* **p-Value**
*

Bisher gibt die Summary-Methode eine Liste von allen dazugehörigen Werten in einer Liste zurück, was extrem unübersichtlich wäre. Demzufolge sorgt die Funktion *print.summary()* für die richtige Einordnung der jeweiligen Werten.
