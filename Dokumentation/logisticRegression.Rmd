...
Die Ergebnisse von der oberen Ableitung hängt davon ab, nach welcher $\beta_j$ mit $j \in (0,1,2,..,p)$ die Ableitung erfolgt wird. 
...
$$
\begin{aligned}
s(\beta) &= \sum_{i = 1}^{n} \Bigg( y_i.\mathbf{x'}_i - \frac{\mathbf{x'}_i.\exp(\sum_{j = 0}^p x_{ij}.\beta_j)}{1 + \exp(\sum_{j = 0}^p x_{ij}.\beta_j)} \Bigg) \\
...
&\mathrel{\overset{(3)}{=}} - \sum_{i = 1}^{n} \Bigg( \mathbf{x'}_i \ .  \Big(\frac{\partial}{\partial \beta} \frac{\exp(\sum_{j = 0}^p x_{ij}.\beta_j)}{1+\exp(\sum_{j = 0}^p x_{ij}.\beta_j)} \Big) \Bigg) \\
\end{aligned}
$$
...
$$
\begin{aligned}
\frac{\partial}{\partial \beta} s(\beta) &= - \sum_{i = 1}^{n} \Bigg[ \mathbf{x'}_i \ . \Bigg( \frac{\mathbf{x'}_i.\exp(\sum_{j = 0}^p x_{ij}.\beta_j).(1+\exp(\sum_{j = 0}^p x_{ij}.\beta_j))-\mathbf{x'}_i.\exp(\sum_{j = 0}^p x_{ij}.\beta_j).\exp(\sum_{j = 0}^p x_{ij}.\beta_j)}{(1+\exp(\sum_{j = 0}^p x_{ij}.\beta_j))^2} \Bigg) \Bigg] \\
&= - \sum_{i = 1}^{n} \Bigg[ \mathbf{x'}_i \ . \frac{\mathbf{x'}_i.\exp(\sum_{j = 0}^p x_{ij}.\beta_j)}{(1+\exp(\sum_{j = 0}^p x_{ij}.\beta_j))^2}  \Bigg] \\
