...
\begin{Shaded}
\begin{Highlighting}[]
\DecValTok{1} \ControlFlowTok{function}\NormalTok{(X, y)}\OperatorTok{:}
\DecValTok{2}\NormalTok{   initialisiere}\OperatorTok{:}\StringTok{ }\ErrorTok{$$}\NormalTok{\textbackslash{}beta_}\DecValTok{0}\OperatorTok{$}\ErrorTok{$}\NormalTok{, maxIteration, i, tolerance, diff, M}
\DecValTok{3}   \ControlFlowTok{while}\NormalTok{ (diff }\OperatorTok{>}\StringTok{ }\NormalTok{tolerance)}
\DecValTok{4}\NormalTok{       betaChange =}\StringTok{ }\ErrorTok{$}\NormalTok{(\textbackslash{}mathbf\{X\}}\StringTok{'\textbackslash{}mathbf\{M\}\textbackslash{}mathbf\{X\})^\{-1\}.\textbackslash{}mathbf\{X\}'}\NormalTok{.(y}\OperatorTok{-}\NormalTok{\textbackslash{}pi)}\OperatorTok{$}
\DecValTok{5}       \DataTypeTok{diff =} 
\DecValTok{6}       \DataTypeTok{i =}\NormalTok{ i }\OperatorTok{+}\StringTok{ }\DecValTok{1}
\DecValTok{7}       \ControlFlowTok{if}\NormalTok{ (i }\OperatorTok{>}\StringTok{ }\NormalTok{maxIteration)}
\DecValTok{8}            \ControlFlowTok{break}
\DecValTok{9}\NormalTok{ end}
\end{Highlighting}
\end{Shaded}
...
\ControlFlowTok{while}\NormalTok{ (diff }\OperatorTok{>}\StringTok{ }\NormalTok{tolerance }\OperatorTok{&}\StringTok{ }\NormalTok{i }\OperatorTok{<}\StringTok{ }\NormalTok{maxIteration) \{}
    ...
    \CommentTok{# berechne die Wahrscheinlichkeit}
...
\NormalTok{    M <-}\StringTok{ }\KeywordTok{diag}\NormalTok{(p }\OperatorTok{*}\StringTok{ }\NormalTok{(}\DecValTok{1} \OperatorTok{-}\StringTok{ }\NormalTok{p))}
    ...
    \CommentTok{# berechne die Änderung von Beta}
...
Die Änderung von \(\beta\) führt dazu, dass nach jeder Iteration
ebenfalls die Eintrittswahrscheinlichkeit sowie \ldots{} Matrix neu
...
\emph{maxLikeEst} mit dem Standardmodell werden die ML-Schätzer und die
Kovarianzmatrix zurückgegeben.
...
\NormalTok{    y <-}\StringTok{ }\KeywordTok{model.response}\NormalTok{(modelFrame)}
    ...
    \CommentTok{# if (!(0 %in% y && 1 %in% y)) \{}
    \CommentTok{#     y <- factor(y, labels = c(0,1))}
    \CommentTok{# \}}
    \CommentTok{# y <- as.numeric(as.character(y))}
...
das Null-Modell erstellt und ebenfalls in das ``Hauptobjekt'' für
leichteren späteten Aufruf gespeichert.
