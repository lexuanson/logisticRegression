...
\CommentTok{#devianceResiduals = -2 * as.numeric(crossprod(y,eta) - sum(log(1 + exp(eta))))    }
...
\NormalTok{logitMod <-}\StringTok{ }\ControlFlowTok{function}\NormalTok{(formula, data) \{}
    ...
    \CommentTok{# initialisiere y (Zielvariable) und X (Matrix enthält alle erklärenden Variablen)}
...
\NormalTok{    nullModell <-}\StringTok{ }\KeywordTok{maxLikeEst}\NormalTok{(}\DataTypeTok{X =} \KeywordTok{matrix}\NormalTok{(}\KeywordTok{rep}\NormalTok{(}\DecValTok{1}\NormalTok{, }\DataTypeTok{times =} \KeywordTok{nrow}\NormalTok{(X)), }\DataTypeTok{ncol =} \DecValTok{1}\NormalTok{), }\DataTypeTok{y =}\NormalTok{ y)}
...
bisherigen Ergebnissen vorgestellt. Das Beispiel befindet sich wieder in
der Extra-Vignette.
...
\NormalTok{print.logitMod <-}\StringTok{ }\ControlFlowTok{function}\NormalTok{(x, ...)\{}
    ...
    \KeywordTok{cat}\NormalTok{(}\StringTok{"}\CharTok{\textbackslash{}n}\StringTok{Degrees of Freedom: "}\NormalTok{, x}\OperatorTok{$}\NormalTok{dfNull, }\StringTok{" Total (i.e. Null); "}\NormalTok{,}
\NormalTok{        x}\OperatorTok{$}\NormalTok{dfRes, }\StringTok{" Residual"}\NormalTok{)}
