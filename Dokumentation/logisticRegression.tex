...
\pi_i = \frac{\exp(\eta_i)}{1 + \exp(\eta_i)} = \frac{\exp(\beta_0 + \beta_1.x_{i2} + \beta_2.x_{i3} + ... + \beta_k.x_{ik} + \epsilon_i)}{1 + \exp(\beta_0 + \beta_1.x_{i2} + \beta_2.x_{i3} + ... + \beta_k.x_{ik} + \epsilon_i)}
...
Wird die Gleichung nach dem linearen Prädikator \(\eta_i\) gelöst, in
dem beide Seite der Gleichung mit \((1 + \exp(\eta_i))\) multipliziert
wird, ergibt sich: \[
